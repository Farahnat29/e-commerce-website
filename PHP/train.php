...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO train (UserID, name, email, phone, arrival_date, departure_date, gender, nationality, message, receive_offers) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("issssssssi", $userid, $name, $email, $phone, $arrivalDate, $departureDate, $gender, $nationality, $message, $receiveOffers);

    if ($stmt->execute()) {
        // Capture the auto-generated trainBookingID
        $trainBookingID = $conn->insert_id;
        // Store trainBookingID in session
        $_SESSION['TrainBookingID'] = $trainBookingID;
